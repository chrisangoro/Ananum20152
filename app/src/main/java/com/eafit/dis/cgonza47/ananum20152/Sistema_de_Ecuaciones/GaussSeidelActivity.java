...
public class GaussSeidelActivity extends ActionBarActivity implements View.OnClickListener {
    ...
    private Metodos_Sistemas_de_Ecuaciones gausseidel;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        di = (DatosIterativos) intent.getSerializableExtra("matriz_di_gausseidel");

        resultados = (TextView) findViewById(R.id.result_gausseidel);
        resultados.setVerticalScrollBarEnabled(true);

        bt_ayuda = (Button) findViewById(R.id.ayuda_gausseidel);
        bt_ayuda.setOnClickListener(this);

        formatter = new DecimalFormat();
        formatter = new DecimalFormat("#.##E0");

        gausseidel = new Metodos_Sistemas_de_Ecuaciones();

        gausseidel.gaussSeidel(di.getA(), di.getB(), di.getA().length, di.getX0()
                , di.getIteraciones(), di.getTolerancia(), di.getAlpha());

        for (int i = 0; i < gausseidel.getResultados_gausseidel().size(); i++) {

            resultados.append(gausseidel.getResultados_gausseidel().get(i));

        }

        tl = (TableLayout) findViewById(R.id.Table_gausseidel);
        ...
        for (int i = 0; i < gausseidel.getIteraciones_gaussidel().size(); i++) {
            ...
            iter.setText(String.valueOf(gausseidel.getIteraciones_gaussidel().get(i)));
            iter.setTextColor(Color.BLACK);
            iter.setPadding(10, 10, 10, 10);
            tr_head.addView(iter);

            for (int j = 0; j < gausseidel.getTam_x0_gausseidel(); j++) {

                x = new TextView(this);
                x.setId((j * 1000000) + 4000010 + i);
                x.setText(String.valueOf(gausseidel.getXs_gausseidel().get(punt)));
                x.setTextColor(Color.BLACK);
                x.setPadding(10, 10, 10, 10);
                tr_head.addView(x);
                punt++;
            ...
            error.setText(formatter.format(gausseidel.getError_gausseidel().get(i)));
            error.setTextColor(Color.BLACK);
            error.setPadding(10, 10, 10, 10);
            tr_head.addView(error);
    ...
    public void onClick(View v) {
        if (v.getId() == R.id.ayuda_gausseidel) {
