...
public class CroutActivity extends ActionBarActivity implements View.OnClickListener {


    private TextView resultado;
    private Matriz matriz;
    private Metodos_Sistemas_de_Ecuaciones crout;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        solo_resultado = (TextView) findViewById(R.id.solo_resultados_crout);
        solo_resultado.setOnClickListener(this);

        bt_ayuda = (Button) findViewById(R.id.ayuda_crout);
        bt_ayuda.setOnClickListener(this);

        resultado = (TextView) findViewById(R.id.result_crout);
        ...
        crout = new Metodos_Sistemas_de_Ecuaciones();

        crout.factorizacionDirectaCrout(matrizA, matrizb, matrizA.length);

        for (int j = 0; j < crout.getSolo_resultado().size(); j++) {

            solo_resultado.append(crout.getSolo_resultado().get(j));

        }

        for (int i = 0; i < crout.getResultados().size(); i++) {

            resultado.append(crout.getResultados().get(i));
    ...
    public void onClick(View v) {
        if (v.getId() == R.id.solo_resultados_crout) {

            solo_resultado.setVisibility(View.INVISIBLE);
            resultado.setVisibility(View.VISIBLE);

        } else if (v.getId() == R.id.ayuda_crout) {

            String mensaje = "El objetivo del m\u00E9todo es encontrar una soluci\u00F3n a " +
                    "un sistema de ecuaciones lineales, basado en la factorizaci\u00F3n LU de " +
                    "la matriz de coeficientes relacionada al sistema.";
